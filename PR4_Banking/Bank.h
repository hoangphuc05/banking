...
private:
	...
	*/
	std::vector<int> find_accounts_by_name(std::string name)
	{
		...
		Customer *cus = find_customer(name);
		
		for (int i = 0; i < accounts.size(); i++) {
			if ((accounts[i]->get_customer()) == cus) {
...
public:
	...
	*/
	Account* add_account(std::string name, std::string account_type) 
	{
		...
		if (cust == NULL)
			return NULL;
		Account *acc = add_account(cust, account_type);
	...
	*/
	void make_deposit(int acct_number, double amt) 
	{
		...
		if (acct) {
			...
			std::cout << "account not found, cannot make a deposit\n";
		}
	}
	...
	*/
	void make_withdrawal(int acct_number, double amt) 
	{
		...
		if (acct) {
			...
			cout << "Account not found, cannot withdraw\n";
	}
